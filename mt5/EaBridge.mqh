...
#ifndef EA_BRIDGE_MQH
#define EA_BRIDGE_MQH
...
bool CEaBridge::SetSocketTimeout(int timeout_ms)
{
   if(m_socket_handle == INVALID_HANDLE)
      return false;
   return SocketSetTimeout(m_socket_handle, timeout_ms);
}
...
string CEaBridge::ReceiveLine()
{
   ...
   while(GetTickCount() < timeout && m_receive_running)
   {
      ...
      if(bytes > 0)
      {
         string chunk = CharArrayToString(buffer, 0, bytes);
         result += chunk;
...
bool CEaBridge::SendLine(string message)
{
   ...
   char buffer[];
   StringToCharArray(message, buffer, 0, WHOLE_ARRAY);
   
   uint sent = SocketSend(m_socket_handle, buffer, StringLen(message));
...
void CEaBridge::ReceiveThread()
{
   ...
   while(m_receive_running && m_is_connected)
   {
